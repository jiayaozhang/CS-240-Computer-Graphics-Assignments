...
/********************************************************************************************/
/*  
/*              rasterize(Model && model )                                    */
/********************************************************************************************/
// rasterize to the specific window size
void Rasterizer::run(Model& model)
{
	...
	// take a small scale to prevent the part of the model length
	//(max(width,height)) from being in the window
...
#pragma omp parallel for //num_threads(NUM_THREADS)
	for (int i = 0; i < vertex_num; ++i)
	{
		...
		// place in the middle of the windoe
